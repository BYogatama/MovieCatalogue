/*
 * Created by Bagus Yogatama on 7/4/19 10:29 AM
 * Copyright (c) 2019 . All rights reserved.
 * Last modified 7/4/19 10:29 AM
 */
...
class FavoriteMovieProvider : ContentProvider() {
    ...
    private val uriMatcher = UriMatcher(UriMatcher.NO_MATCH).apply {
        addURI(AUTHORITIES, "favourites", CODE_MOVIES_DIR)
        addURI(AUTHORITIES, "favourites/#", CODE_MOVIES_ITEM)
    }
    ...
    override fun insert(uri: Uri, values: ContentValues?): Uri? {
        when (uriMatcher.match(uri)) {
            ...
            CODE_MOVIES_ITEM -> throw IllegalArgumentException("Invalid URI, cannot insert with ID: $uri")
    ...
    override fun query(
        ...
        return when (uriMatcher.match(uri)) {
            CODE_MOVIES_DIR -> {
                val cursor: Cursor = favouritesDao.getFavouriteMoviesCursor()
                cursor.setNotificationUri(context.contentResolver, uri)
                cursor
            }
            CODE_MOVIES_ITEM -> {
                val cursor = favouritesDao.getFavoritedMoviesCursor(ContentUris.parseId(uri))
                cursor.setNotificationUri(context.contentResolver, uri)
                cursor
            }
            else -> throw IllegalArgumentException("Unknown URI: $uri")
    ...
    override fun getType(uri: Uri): String? {
        return when (uriMatcher.match(uri)) {
            CODE_MOVIES_DIR -> "vnd.android.cursor.dir/$AUTHORITIES.favourites"
            CODE_MOVIES_ITEM -> "vnd.android.cursor.item/$AUTHORITIES.favourites"
            else -> throw IllegalArgumentException("Unknown URI: $uri")
        }
    }
